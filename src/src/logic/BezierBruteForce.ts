...
for (let i = 1; i<=numSteps; i++){ //Mencari posisi titik selain titik awal dan akhir
  const t: number = stepSize*i;
  let xTemp: number = 0;
  let yTemp: number = 0;
  for(let j = 0; j<=nBezier; j++){
    xTemp += (pascalSeries[j])*((1-t)**(nBezier-j))*(t**j)*points[j].x;
    yTemp += (pascalSeries[j])*((1-t)**(nBezier-j))*(t**j)*points[j].y;
  }
  const pointTemp: Point = {x: xTemp, y: yTemp};
  returnArr.push(pointTemp);
}
...
console.log(QuadraticBezierBruteForce([{x: 0, y: 15}, {x: 15, y: 0}, {x:15, y:15}], 3))
